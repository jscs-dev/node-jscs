...
describe('rules/disallow-identifier-names', function() {
    ...
    it('should report illegal identifier names', function() {
        expect(checker.checkString('temp = 1;'))
            .to.have.one.error.from('disallowIdentifierNames');
        expect(checker.checkString('var temp = 1;'))
            .to.have.one.error.from('disallowIdentifierNames');
        expect(checker.checkString('let temp = 1;'))
            .to.have.one.error.from('disallowIdentifierNames');
        expect(checker.checkString('const temp = 1;'))
            .to.have.one.error.from('disallowIdentifierNames');
        expect(checker.checkString('function fn(foo) {var temp = 1;}'))
            .to.have.errors.from('disallowIdentifierNames').and.count.equals(2);
        expect(checker.checkString('fn(temp)'))
            .to.have.one.error.from('disallowIdentifierNames');
        expect(checker.checkString('if(temp){}'))
            .to.have.one.error.from('disallowIdentifierNames');
        expect(checker.checkString('for(temp = 0; temp<1; temp++){temp;}'))
            .to.have.errors.from('disallowIdentifierNames').and.count.equals(4);
        expect(checker.checkString('obj[foo] = 1;'))
            .to.have.one.error.from('disallowIdentifierNames');
        expect(checker.checkString('obj["foo"] = 1;'))
            .to.have.one.error.from('disallowIdentifierNames');
        expect(checker.checkString('obj.foo = 1;'))
