...
describe('rules/require-line-break-after-variable-assignment', function() {
    ...
    describe('"line-break-after-variable-assignment"', function() {
        ...
        it('should report when constants are defined and assigned on the same line.', function() {
            assert(checker.checkString('const x = 1; const y=2;').getErrorCount() === 1);
        });

        it('should not report when constants are defined on separate lines.', function() {
            assert(checker.checkString('const x = 1;\nconst y=2;').isEmpty());
        });
