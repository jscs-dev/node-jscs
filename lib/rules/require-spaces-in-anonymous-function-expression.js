 ...
 *      - `"allExcept"` may be an Array containing `"shorthandMethods"`, or
 *        the Boolean `true` to enable all configuration exceptions. If
 *        `"shorthandMethods"` is provided, spaces will not be required for
 *        ES6 method shorthand declarations.
 *
 * #### Example
 *
 * ```js
        ...
        if ('allExcept' in options) {
            if (typeof options.allExcept === 'object') {
                ...
                options.allExcept.forEach(function(except) {
                    if (except === 'shorthandMethods') {
                        this._exceptions.shorthandMethods = true;
                    } else {
                        assert(false, this.getOptionName() + ' option requires ' +
                        '"allExcept" to be an array containing "shorthandMethods"');
                    }
                }, this);
            } else {
                assert(
    ...
    check: function(file, errors) {
        ...
        file.iterateNodesByType(['FunctionExpression'], function(node) {
            ...
            if (parent.method || parent.type === 'MethodDefinition') {
                ...
                if (exceptions.shorthandMethods) {
                    return;
                }
            }
