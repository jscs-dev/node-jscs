    ...
    check: function(file, errors) {
        ...
        function generateIndentations() {
            ...
            file.iterateNodesByType([
                ...
                linesToCheck[node.loc.start.line - 1] = false;
                var additionalIndents = getIndentationFromLine(node.loc.start.line - 1);

                for (var x = node.init.loc.start.line + 1; x <= node.init.loc.end.line; x++) {
                    lineIndentation[x - 1] += additionalIndents;
                }
            });
        }
