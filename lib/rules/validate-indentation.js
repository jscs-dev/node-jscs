 ...
 * ```
 *
 * ##### Valid example for mode `{ "value": "\t", "allExcept": { "emptyLines": true } }`
 * ```js
 * if (a) {
 *     b=c;
 *     function(d) {
 ...
 * ```
 *
 * ##### Invalid example for mode `{ "value": "\t", "allExcept": { "emptyLines": true } }`
 * ```js
 * if (a) {
 *     b=c;
 *     function(d) {
...
var Indent = require('../indentation');
...
module.exports.prototype = {
    ...
    check: function(file, errors) {
        ...
        function markChildren(node) {
            Indent.getChildren(node).forEach(function(childNode) {
        ...
        function checkIndentations() {
            ...
            linesToCheck.forEach(function(line, i) {
                ...
                var expectedIndentation = indent.getExpectedIndentation(line, actualIndentation);
                ...
                if (line.push.length) {
                    indent.pushExpectedIndentations(line, linesToCheck, actualIndentation);
                }
            });
        }
        ...
        function generateIndentations() {
            ...
            file.iterateNodesByType('ObjectExpression', function(node) {
                if (!Indent.isObjectIndentable(node)) {
                    return;
                }
            ...
            file.iterateNodesByType('SwitchStatement', function(node) {
                ...
                var indents = 1;
                var children = Indent.getChildren(node);
        ...
        var indent = new Indent(indentChar, indentSize);
        ...
        var linesToCheck = file.getLines().map(function(line) {
            return {
                ...
                pop: [],
                check: false,
                indentation: indent.getIndentationFromLine(line),
                empty: line.match(/^\s*$/)
            };
        });
