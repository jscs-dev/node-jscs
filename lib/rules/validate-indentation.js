...
function isBlockOnTheSameLineWithCase(element) {
    ...
    while (currentSwitchElement) {
        ...
        if (currentSwitchElement.newlineCount > 0) {
            break;
        }
...
module.exports.prototype = {
    ...
    _getElementDirectIndent: function(node) {
        var whitespaceToken = node.previousToken;
        if (whitespaceToken.isWhitespace && whitespaceToken.newlineCount > 0) {
            var endTokenLines = whitespaceToken.sourceCodeLines;
    ...
    _checkNode: function(parentElement, errors, initialIndent, options) {
        ...
        while (element) {
            if (element.isToken) {
                ...
                if (element.isWhitespace && (element.newlineCount > 0 || isFirstWhitespace)) {
                    var lines = getLineData(element.sourceCode);
                    ...
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        if (line.indexOf(this._nonIndentChar) !== -1) {
                            errors.add('Invalid indentation character: ' + this._nonIndentChar, element, offset);
                        }
                        ...
                        if (!checkBlockIndentation) {
                            ...
                            var nextToken = element.nextToken;
                            if (nextToken &&
                                nextToken.isToken &&
                        ...
                        if (checkBlockIndentation || checkForStatement || checkForKeyword) {
                            ...
                            if (isLastLine && nextSibling && nextSibling.isComment) {
                                ...
                                var brace = nextSibling.nextNonWhitespaceToken;
    ...
    check: function(file, errors) {
        ...
        var firstWhitespace;
        if (program.firstToken.isWhitespace) {
            firstWhitespace = program.firstToken;
        }
    ...
    _fix: function(file, error) {
        ...
        if (lineItemToFix) {
            ...
            if (adjustElement && adjustElement.newlineCount > 0) {
                var currentToken = adjustElement.firstToken;
                var lastToken = adjustElement.lastToken;
                while (true) {
                    var nextToken = currentToken.nextToken;
                    if (currentToken.isWhitespace && currentToken.newlineCount > 0) {
                    ...
                    if (currentToken.isComment && currentToken.newlineCount > 0) {
                        var prev = currentToken.previousToken;
                        var commentIndent = 0;
                        if (prev.isWhitespace && prev.newlineCount > 0) {
                            commentIndent = prev.sourceCodeLines.concat().pop().length;
                        }
