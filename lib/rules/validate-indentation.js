...
module.exports.prototype = {
    ...
    check: function(file, errors) {
        function markPushLine(line, indents) {
            linesToCheck[line].push = [indents];
        }
        ...
        function markPush(pushNode, indents) {
            markPushLine(pushNode.loc.start.line - 1, indents);
        }
        ...
        function pushExpectedIndentations(line, actualIndentation) {
            ...
            var expectedIndentation = actualIndentation + (indentSize * indents);
            var pushValue = expectedIndentation;
            ...
            if (line.pushAltLine !== null) {
                pushValue = [
                    expectedIndentation,
                    getIndentationFromLine(line.pushAltLine) + (indentSize * indents)
                ];
            }

            line.push.forEach(function() {
                indentStack.push(pushValue);
            });
        }
        ...
        function generateIndentations() {
            ...
            file.iterateNodesByType('VariableDeclaration', function(node) {
                ...
                if (startLine !== declStartLine || !linesToCheck[startLine].push.length) {
                    return;
                }

                newIndentLine = decls.length > 1 ? decls[1].loc.start.line : decls[0].loc.end.line;

                actualIndents = getIndentationFromLine(startLine);
                newIndents = getIndentationFromLine(newIndentLine - 1);

                markPushLine(startLine, ((newIndents - actualIndents) / indentSize) + 1);
            });
        }
