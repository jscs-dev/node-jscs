...
module.exports.prototype = {
    ...
    check: function(file, errors) {
        ...
        file.iterateNodesByType(['BlockStatement', 'ClassBody', 'SwitchStatement'], function(node) {
            var blockType = getBlockType(node);
            if (blockType !== 'switch' && isBareBlock(node)) {
                return;
            }

            if (setting === true || setting.indexOf(blockType) !== -1) {
                var openingBrace;
                if (blockType === 'switch') {
                    openingBrace = file.findNextToken(file.getLastNodeToken(node.discriminant), 'Punctuator', '{');
                } else {
                    openingBrace = file.getFirstNodeToken(node);
                }
