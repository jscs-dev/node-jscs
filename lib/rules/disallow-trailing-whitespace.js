    ...
    check: function(file, errors) {
        ...
        for (var i = 0, l = lines.length; i < l; i++) {
            if (lines[i].match(/\s$/) && !(ignoreEmptyLines && lines[i].match(/^\s*$/))) {
                ...
                if(precendingToken.isComment && precendingToken.loc.start.line <= (i+1) && precendingToken.loc.end.line >= (i+1)) {
                ...
                if(typeof targetToken !== 'undefined') {
                    ...
                    for(var j = 0, whitespace = targetLine.charAt(j); whitespace.match(/\s/); j++, whitespace = targetLine.charAt(j)) {
