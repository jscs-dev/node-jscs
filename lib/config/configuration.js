...
function Configuration() {
     ...
     * @type {Array}
     */
    this._defaultBundlePatterns = ['**/*.+(htm|html|xhtml)'];
    ...
    /**
     * Default ignore patterns; rewritten if user provides his own.
     *
     * @protected
     * @type {Array}
     */
    this._defaultIgnorePatterns = ['.git/**', '*/node_modules/**'];

    /**
     * The internal GitKeeper instance for processing 'git-ignore' rules.
     *
     * @protected
     * @type {Object}
     */
    this._keeper = new GitKeeper({
        basePath: this._basePath,
        filenames: [utils.IGNORE_FILENAME]
    });
...
Configuration.prototype.getProcessedConfig = function() {
    ...
    result.keeper = this._keeper;
    result.fileExtensions = this._fileExtensions;
    result.extract = this._bundlePatterns;
...
/**
 * Returns the internal GitKeeper instance (may be superfluous).
 *
 * @returns {Object}
 */
Configuration.prototype.getKeeper = function() {
    return this._keeper;
};
...
Configuration.prototype.isFileExcluded = function(filePath) {
    return this._keeper.bans(filePath);
};
...
/**
 * Returns the bundle patterns.
 *
 * @returns {String[]}
 */
Configuration.prototype.getBundlePatterns = function() {
    return this._bundlePatterns;
};
...
Configuration.prototype.shouldExtractFile = function(filePath) {
    ...
    return this._bundleMatchers.some(function(matcher) {
...
Configuration.prototype._processConfig = function(config) {
    if (Array.takes(config, '*')) {
        this._keeper.add(config);
        return;
    }
    ...
    if (this._basePath === defaults.cwd && options.configPath) {
        ...
        this._keeper.basePath = options.configPath;
    }
    ...
    // Set default patterns if there is no presets that could define their own
    } else if (!options.hasOwnProperty('preset')) {
        this._loadExcludedFiles(this._defaultIgnorePatterns);
    }
 ...
 * @protected
 */
Configuration.prototype._loadExcludedFiles = function(patterns) {
    assert(Array.isArray(patterns), '`excludeFiles` option requires array value');
    this._keeper.add(patterns);
};
 ...
 * @protected
 */
Configuration.prototype._loadExtract = function(patterns) {
    if (patterns === true) {
        patterns = this._defaultBundlePatterns;
    } else if (patterns === false) {
        patterns = [];
    }

    assert(Array.isArray(patterns), '`extract` option should be array of strings');
    this._bundlePatterns = patterns.slice();
    this._bundleMatchers = this._bundlePatterns.map(function(fileMask) {
