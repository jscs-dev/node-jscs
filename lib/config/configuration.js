...
Configuration.prototype._processConfig = function(config) {
    ...
    // Base path
    if (this._basePath === defaultCWD && options.configPath) {
        assert(
    ...
    if (options.hasOwnProperty('extract')) {
        assert(Array.isArray(options.extract), '`extract` option requires array value');
        this._loadExtract(options.extract.concat(['**/*.+(htm|html|xhtml)']));
    }
