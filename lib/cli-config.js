...
// Configuration sources in reverse priority order.
var configs = ['.jscs.yaml', '.jscs.json', '.jscsrc', 'package.json'];
var directoryArr = ['HOME', 'HOMEPATH', 'USERPROFILE'];
...
function findup (patterns, options, fn) {
    ...
    do {
        file = patterns.filter("r", function (pattern) {
    ...
    if (fs.existsSync(configPath)) {
        ...
        if (ext === '.js') {
            ...
            switch (ext) {
                ...
                default:
                    if (config + ext === utils.IGNORE_FILENAME) {
                        content = Object.bake(data.lines, {
                            from: config
                        });
                        break;
                    }
...
exports.load = function (config, cwd) {
    ...
    for (var i = directoryArr.length - 1; i >= 0; --i) {
        directory = process.env[directoryArr[i]];

        for (var j = configs.length - 1; j >= 0; --j) {
            content = this.getContent(configs[j], directory);
            if (content) {
                return content;
            }
        }

        content = this.getContent(utils.IGNORE_FILENAME, directory);
        if (content) {
            return content;
        }
    }
