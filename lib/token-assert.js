...
TokenAssert.prototype.linesBetween = function(options) {
    ...
    while (currentToken && currentToken !== nextToken) {
        linesBetween += currentToken.lineBreakCount;
...
TokenAssert.prototype._augmentLineCount = function(options, lineCount) {
    ...
    var previousCodeToken = token.previousCodeToken;
    if (lineCount === 0) {
        if (options.stickToPreviousToken) {
            var nextToken = token.nextNonWhitespaceToken;
            nextToken.whitespaceBefore = token.whitespaceBefore;
        }
        var currentToken = token.previousToken;
        while (currentToken.isWhitespace) {
            var tokenToRemove = currentToken;
            currentToken = tokenToRemove.previousToken;
            if (currentToken === previousCodeToken) {
                tokenToRemove.parentElement.replaceChildren(
                    new Token('Whitespace', ' '),
                    tokenToRemove,
                    tokenToRemove
                );
            } else {
                tokenToRemove.parentElement.removeChild(tokenToRemove);
            }
        }
        return;
    }
