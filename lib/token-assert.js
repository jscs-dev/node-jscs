...
TokenAssert.prototype.linesBetween = function(options) {
    ...
    var blankLines = Math.abs(token.loc.end.line - nextToken.loc.start.line) - 1;
    ...
    if (atLeast !== undefined && blankLines < atLeast) {
        emitError('at least ' + atLeast);
    }

    if (atMost !== undefined && blankLines > atMost) {
        emitError('at most ' + atMost);
    }

    if (exactly !== undefined && blankLines !== exactly) {
