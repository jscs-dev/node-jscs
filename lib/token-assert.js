 ...
 * @param {Number} [options.lines=1] Amount of lines between tokens.
...
TokenAssert.prototype.differentLine = function(options) {
    ...
    var gap = options.hasOwnProperty('lines') ? options.lines : 1;
    ...
    if ((!allowExtra && lineDifference !== gap) ||
        (allowExtra && lineDifference < gap)) {
        this.emit('error', {
