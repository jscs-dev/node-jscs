 ...
 *
 * @param {Object} options.token
 * @param {Object} options.subjectToken
 * @param {String} [options.message]
 */
TokenAssert.prototype.sameLine = function(options) {
    var token = options.token;
    var subjectToken = options.subjectToken;

    if (!token || !subjectToken) {
        return;
    }

    if (token.loc.end.line !== subjectToken.loc.start.line) {
        this.emit('error', {
            message: options.message || token.value + ' and ' + subjectToken.value + ' should be on the same line',
            line: token.loc.end.line,
            column: token.loc.end.column
        });
    }
...
TokenAssert.prototype.differentLine = function(options) {
    ...
    if (token.loc.end.line === subjectToken.loc.start.line) {
        this.emit('error', {
            message: options.message || token.value + ' and ' + subjectToken.value + ' should be on different lines',
            line: token.loc.end.line,
            column: token.loc.end.column
        });
    }
