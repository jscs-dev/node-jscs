...
TokenAssert.prototype.linesBetween = function(options) {
    ...
    // Only attempt to remove or add lines if there are no comments between the two nodes
    // as this prevents accidentally moving a valid token onto a line comment ed line
    var fixed = token.nextNonWhitespaceToken === nextToken;

    var linesBetween = 0;
    var currentToken = token.nextToken;

    // check the token (firstToken) not the node
    while (currentToken && currentToken !== nextToken.firstToken) {
        linesBetween += currentToken.newlineCount;
...
TokenAssert.prototype._augmentLineCount = function(options, lineCount) {
    ...
    var previousToken = token.previousNonWhitespaceToken;
    if (lineCount === 0) {
        if (options.stickToPreviousToken) {
            applyWhitespaceBetweenTokens(
                token,
                token.nextNonWhitespaceToken,
                getWhitespaceBetweenTokens(previousToken, token)
            );
        }

        applyWhitespaceBetweenTokens(previousToken, token, ' ');
        return;
    }

    // TODO: 3.0 Augment line count
    //this._updateWhitespaceByLine(token, function(lines) {
...
/**
 * Returns whitespace between two tokens.
 *
 * @param {String} token
 * @param {String} nextToken
 * @returns {string}
 */
function getWhitespaceBetweenTokens(token, nextToken) {
    var whitespace = '';
    var currentToken = token.nextToken;
    while (currentToken !== nextToken) {
        if (!currentToken.isWhitespace) {
            throw new Error('Non-whitespace token found.');
        }
        whitespace += currentToken.value;
        currentToken = currentToken.nextToken;
    }
    return whitespace;
}
...
/**
 * Applies whitespace between two tokens.
 *
 * @param {Token} token
 * @param {Token} nextToken
 * @param {String} whitespace
 */
function applyWhitespaceBetweenTokens(token, nextToken, whitespace) {
    var replacementToken = new Token('Whitespace', whitespace);
    var tokensBetween = [];
    var currentToken = token.nextToken;
    while (currentToken !== nextToken) {
        if (!currentToken.isWhitespace) {
        ...
        tokensBetween.push(currentToken);
        currentToken = currentToken.nextToken;
    }
    if (tokensBetween.length > 0) {
        var tokenToDelete;
        for (var i = 1; i < tokensBetween.length; i++) {
            tokenToDelete = tokensBetween[i];
            tokenToDelete.parentElement.removeChild(tokenToDelete);
        }

        var tokenToReplace = tokensBetween[0];
        tokenToReplace.parentElement.replaceChildren(
            replacementToken,
            tokenToReplace,
            tokenToReplace
        );
    } else {
        var commonAncestor = getTokenCommonAncestor(token, nextToken);
        commonAncestor.secondTokenAncestor.parentElement.insertChildBefore(
            replacementToken,
            commonAncestor.secondTokenAncestor
        );
    }
}

/**
 * Returns common ancestor for two tokens.
 *
 * @param {Token} firstToken
 * @param {Token} secondToken
 * @returns {{commonAncestor: Element, firstTokenAncestor: Element, secondTokenAncestor: Element}}
 */
function getTokenCommonAncestor(firstToken, secondToken) {
    var firstTokenParents = getTokenParents(firstToken);
    var secondTokenParents = getTokenParents(secondToken);
    var minParentLength = Math.min(firstTokenParents.length, secondTokenParents.length);
    var firstTokenAncestor;
    var secondTokenAncestor;
    for (var i = 0; i < minParentLength; i++) {
        firstTokenAncestor = firstTokenParents[i];
        secondTokenAncestor = secondTokenParents[i];
        if (firstTokenAncestor !== secondTokenAncestor) {
            return {
                firstTokenAncestor: firstTokenAncestor,
                secondTokenAncestor: secondTokenAncestor
            };
        }
    }
    throw new Error('Could not find common ancestor for tokens');
}

/**
 * Returns token parent list.
 *
 * @param {Token} token
 * @returns {Element[]}
 */
function getTokenParents(token) {
