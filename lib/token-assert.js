 ...
 * @param {Number} [options.lines=0] Amount of blank lines required between tokens.
 * @param {Boolean} [options.allowExtra=false] Allow more than specified number of lines.
 */
TokenAssert.prototype.differentLine = function(options) {
    ...
    var gap = options.hasOwnProperty('lines') ? options.lines : 0;
    var allowExtra = options.hasOwnProperty('allowExtra') ? options.allowExtra : false;

    var errorMessage = token.value + ' and ' + nextToken.value + ' should be ';
    if (gap === 0) {
        errorMessage += 'on different lines';
    } else {
        errorMessage += 'separated by ' + gap + ' lines';
    }

    var blankLinesBetween = Math.abs(token.loc.end.line - nextToken.loc.start.line) - 1;

    if ((!allowExtra && blankLinesBetween !== gap) ||
        (allowExtra && blankLinesBetween < gap)) {
        this.emit('error', {
