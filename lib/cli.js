...
try {
    ...
    if (!/^Error: Cannot find module/.test(e)) {
        throw e;
    }
}
...
function cli(program) {
    ...
    // It is more logical to set maxErrors to Infinity since
    // it doesn't makes sense to limit error output with enabled "fix" option
    if (program.fix) {
