        ...
        if (currentToken.isComment) {
            ...
            if (blockMatch) {
                ...
                if (lineMatch) {
                    ...
                    index.push(null);
                    var ignoreToken = currentToken.previousToken;
                    var i = index.length - 1;
                    while (ignoreToken) {
                        i--;
                        index[i] = assign({}, index[i], ignoreState);
                        if (ignoreToken.newlineCount > 0) {
                            break;
                        }
                        ignoreToken = ignoreToken.previousToken;
                    }
                    ignoreToken = currentToken.nextToken;
                    while (ignoreToken) {
                        index.push(ignoreState);
                        if (ignoreToken.newlineCount > 0) {
                            break;
                        }
                        ignoreToken = ignoreToken.nextToken;
                    }
...
PragmaIndex.prototype.isRuleEnabled = function(ruleName, element) {
    var pos = this._tokens.indexOf(element.firstToken);
    if (pos !== -1) {
