...
JsFile.prototype = {
    ...
    setWhitespaceBefore: function(token, whitespace) {
        ...
        if (prevToken && whitespace === '' && prevToken.isWhitespace) {
            prevToken.remove();
            return;
        }

        var fragment = new cst.Fragment(new Token('Whitespace', whitespace));
        var added = false;
        var parent = token.parentElement;
        var grandpa = parent.parentElement;

        while (!added && parent.parentElement) {
            try {
    ...
    getWhitespaceBefore: function(token) {
        return token.previousWhitespaceToken ? token.previousWhitespaceToken.sourceCode : '';
    },
