...
JsFile.prototype = {
    ...
    _buildTokenIndex: function() {
        ...
        if (this.getDialect() === 'es6') {
            var lastToken = this._tree.tokens[0];
            this._tree.tokens = this._tree.tokens.filter(function(token, i) {
                if (i === 0) {
                    return true;
                }

                var repeatedToken = lastToken.range[0] === token.range[0] && lastToken.range[1] === token.range[1];
                lastToken = token;
    ...
    getLinesWithCommentsRemoved: function(errors) {
        ...
        this.getComments().reverse().forEach(function(comment) {
